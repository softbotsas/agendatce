$(document).ready(function () {
    ...
    function inicializarDatatable(datos) {
      ...
      datatable = $('#kt_datatable_guia_2_1').KTDatatable({
        ...
        columns: [
          ...
          },
          {
            ...
            title: 'Tot Pagos',
            template: function(row) {
              const totalFormateado = `$${Number(parseFloat(row.pagos)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
              return totalFormateado;
            }
          },
          {
            ...
            title: 'Tot Comision',
            template: function(row) {
              const totalFormateado = `$${Number(parseFloat(row.comisiones)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
              return totalFormateado;
            }
          },
          {
            ...
            title: 'Resto',
            template: function(row) {
              const totalFormateado = `$${Number(parseFloat(row.pendiente)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}`;
              return totalFormateado;
            }
          }
    ...
    function cargarGuias() {
      $.ajax({
        url: '/api/guias0', // Asegúrate de que esta URL coincide con tu ruta de API
        method: 'GET',
   ...
   // cargarGuias();
    ...
    $('#informacion_comi').click(function () {
      ...
      $.ajax({
        url: '/guia_pdt_registrar_agencia_eficiencia', // Asegúrate de que esta URL es correcta
        method: 'GET',
        data: {
          desde: desde,
          hasta: hasta,
