...
function exportarPDF() {
  ...
  if (!movTrailer || movTrailer.length === 0) {
    Swal.fire('Error', 'No hay datos para exportar en PDF.', 'error');
    return;
  }
  
  // Definir los encabezados para la tabla en el PDF
  const columns = ["N° / Creado", "Cajas Nro", "Conductor"];
  
  // Mapear los datos de movTrailer a un array de arrays
  const rows = movTrailer.map(item => {
    // Se asume que "item.fecha" es una fecha y "item.guia.nro_guia" está en el objeto guía.
    ...
    return [
      item.guia ? `${item.guia.nro_guia} - ${fechaFormateada}` : "",
      item.cant_caja || "",
  ...
  doc.setFontSize(14);
  doc.text("Reporte de Guías", 40, 40);
  
  // Usar autoTable para insertar la tabla en el PDF
  doc.autoTable({
    head: [columns],
    body: rows,
    startY: 60,
  ...
  doc.save("reporte_guias.pdf");
   ...
   var data = movTrailer;     // Fuente de datos (si la necesitas para el datatable)
    ...
    console.log("movTrailer:", movTrailer);
    ...
    var KTDatatableRemoteAjaxDemo = function() {
      var demo = function() {
        ...
        var datatable = $('#kt_datatable_trailer_guia').KTDatatable({
          data: {
            type: 'local',
      ...
      //</button>
        ...
        $('#kt_datatable_trailer_guia').on('click', '.enviar-notif', function() {
          // Obtenemos el índice
 ...
 $('#contenido_bsucar').on('input', function () {
  ...
  if (valor.length >6) { 
      const nroGuia = $(this).val().trim(); // Obtener el valor del input
      if (!nroGuia) {
          alert('Por favor, ingrese un número de guía.');
          return;
      }

      console.log('Número de guía a buscar:', nroGuia);

      // Llamada AJAX para asignar la guía al tráiler
      $.ajax({
          url: '/api/asignar_guias_trailer_individual',
          method: 'POST',
          contentType: 'application/json',
          data: JSON.stringify({ 
              nro_guia: nroGuia,
              trailer: trailer // Asegúrate de que `trailer` esté definido en el contexto
          }),
          success: function (response) {
           Swal.fire({
          icon: 'success',
          ...
          timer: 500, // Se cierra automáticamente después de 1 segundo (1000 ms)
          ...
          error: function (jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.status === 404) {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: jqXHR.responseJSON?.message || 'La guía ya fue registrada previamente en un tráiler.',
                timer: 500, // 1 segundo
                showConfirmButton: false
              });
            } else {
        ...
        // =============================
  $('#kt_datatable_trailer_guia').on('click', '.toggle-guides', function() {
        ...
        $(document).on('change', '#seleccionarTodasGuias', function() {
          ...
          // Busca los checkboxes dentro de la lista específica
          $('#lista_agencias_disponibles .guia-checkbox').prop('checked', isChecked);
        });

        // Handler para los checkboxes individuales de cada guía
        ...
        $('#asignarGuiasBtn').on('click', function() {
          ...
          $('#lista_agencias_disponibles .guia-checkbox:checked').each(function() {
              guiaIds.push($(this).val()); // El valor del checkbox es el guia._id
          });
      
          if (guiaIds.length === 0) {
              Swal.fire('Atención', 'Debes seleccionar al menos una guía para asignar.', 'warning');
              return;
          }
          console.log('Guía IDs seleccionadas a asignar:', guiaIds);
      
          // Asegúrate que las variables 'trailer' y 'conductor_carga' estén definidas y accesibles aquí.
          // 'conductor_carga' se asigna en la llamada AJAX anterior.
          // 'trailer' debe estar definida en tu scope.
          // Ejemplo: const trailer = $('#id_del_selector_trailer').val(); 
          //      o una variable global/de scope superior si ya está definida.
      
          $.ajax({
              url: '/api/asignar_guia_trailer',
              method: 'POST',
              contentType: 'application/json',
              data: JSON.stringify({
                  guias: guiaIds,
                  trailer: trailer,           // Reemplaza con la forma en que obtienes el ID del tráiler
                  conductor_carga: conductor_carga // Esta viene de la llamada AJAX anterior
              }),
              success: function(response) {
                  Swal.fire(
                      '¡Éxito!',
                      response.message || 'Las guías han sido asignadas a este tráiler.',
                      'success'
                  ).then(() => location.reload()); // Recargar la página tras el éxito
              },
              error: function(jqXHR, textStatus, errorThrown) {
                  console.error('Error AJAX al asignar:', jqXHR.responseText, textStatus, errorThrown);
                  ...
                  if (jqXHR.responseJSON && jqXHR.responseJSON.message) {
                      ...
                      errorMessage = 'Una o más guías ya fueron registradas previamente en un tráiler.';
                  }
        ...
        $('#kt_datatable_search_type2').on('change', function() {
          ...
          $.ajax({
              url: '/api/guias/conductor/' + userId,
              method: 'GET',
              dataType: 'json',
              success: function(response) {
                  const responseGuias = response.guias;
                  conductor_carga = response.userId; // Asignar el userId retornado (asegúrate que conductor_carga esté accesible)
                  console.log('Guías filtradas:', responseGuias);
      
                  // guias_asignar_trailer = responseGuias; // Esta variable global ya no será la fuente directa para los IDs a asignar
                  $('#loadingSpinner').hide();
      
                  if (!responseGuias || responseGuias.length === 0) {
                      $('#lista_agencias_disponibles').append('<li style="padding: 10px 15px; text-align: center; color: #888;">No hay guías disponibles para este conductor.</li>');
                  } else {
                      ...
                      $('#asignarGuiasBtnContainer').show(); // Mostrar el botón asignar si hay guías
                  }
              },
              error: function(err) {
                  $('#loadingSpinner').hide();
