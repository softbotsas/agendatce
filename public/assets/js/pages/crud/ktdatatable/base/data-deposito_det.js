    ...
    // let movTrailer;      // Asumimos que se inyecta o define en otro lugar
    // let trailer;         // Igualmente se inyecta o define
    // let conductor_carga; // Se definirá cuando el usuario cambie el select
    // let guias_asignar_trailer = [];
  
    var datatable;         // Instancia para el datatable
   var data = movDeposito;     // Fuente de datos (si la necesitas para el datatable)
  
    // Inicializa la tabla usando "movTrailer" (asegúrate de que exista)
    ...
    var paisesAgrupados = agruparPorPais(data);
   ...
   //inicializarDatatable(movDeposito);
  
    console.log("movTrailer:", movDeposito);
    ...
    var KTDatatableRemoteAjaxDemo = function() {
      var demo = function() {
        ...
        var datatable = $('#kt_datatable_deposito_guia').KTDatatable({
          data: {
            type: 'local',
      ...
      //</button>
        ...
        $('#kt_datatable_deposito_guia').on('click', '.enviar-notif', function() {
          // Obtenemos el índice
    ...
    function imprimirListadoConJsPDF(index) {
      ...
      window.open(pdfURL, "_blank");
        ...
        $('#contenido_bsucar').on('input', function () {
          ...
          // Configura un nuevo timer de 500 ms
           const wait = setTimeout(() => {
            ...
            if (valor.includes('/')) {
              ...
              // Verificamos que se hayan ingresado ambos datos
              if (!nro_guia || !caja) {
              ...
              Swal.fire('El formato de la Etiqueta de esta caja  es inválido.', 'error');
          ...
          // Guardamos el timer para poder cancelarlo si el usuario sigue escribiendo
        ...
        // =============================
        $('#kt_datatable_deposito_guia').on('click', '.toggle-guides', function() {
        ...
        $('#kt_datatable_search_type2').on('change', function() {
          const userId = $(this).val(); // In this context, userId might represent the trailer ID based on the URL
          console.log('Elemento seleccionado (e.g., Trailer ID):', userId);
  
          $('#loadingSpinner').show();
          $('#lista_agencias_disponibles').empty(); // Clear previous list
          $('#asignarGuiasBtnContainer').hide();   // Hide assign button initially
  
          $.ajax({
              url: '/api/guias/trailer/' + userId, // URL to get guides for the selected trailer/element
              method: 'GET',
              dataType: 'json',
              success: function(response) {
                  // Assuming response is { guias: [...], someId: ... }
                  // 'conductor_carga' was used in the previous example, adapt if 'response.userId' means something else here
                  // For example, if it's the trailer ID itself:
                  // const currentTrailerId = response.userId; // or response.trailerId if the API returns that
  
                  const responseGuias = response.guias;
                  console.log('Guías filtradas para el elemento ' + userId + ':', responseGuias);
  
                  // 'guias_asignar_trailer' might be a global or scoped variable you update
                  // It's less critical now for getting IDs to assign, as checkboxes will be the source.
                  // guias_asignar_trailer = responseGuias;
  
                  $('#loadingSpinner').hide();
  
                  if (!responseGuias || responseGuias.length === 0) {
                      $('#lista_agencias_disponibles').append('<li style="padding: 10px 15px; text-align: center; color: #888;">No hay guías en este tráiler/elemento.</li>');
                  } else {
                      ...
                      // Populate list with individual guides and their checkboxes
                      ...
                      $('#asignarGuiasBtnContainer').show(); // Show the assign button
                  }
              },
              error: function(err) {
                  $('#loadingSpinner').hide();
                  console.error('Error al obtener guías para el elemento ' + userId + ':', err);
                  $('#lista_agencias_disponibles').append('<li style="padding: 10px 15px; color: red; text-align: center;">Error al cargar guías.</li>');
              }
          });
      });
  
      // --- Event Handlers for Checkbox Functionality ---
  
      // Handler for the "Seleccionar/Deseleccionar Todas" checkbox
      $(document).on('change', '#seleccionarTodasGuias', function() {
          const isChecked = $(this).prop('checked');
          $('#lista_agencias_disponibles .guia-checkbox').prop('checked', isChecked);
      });
  
      // Handler for individual guide checkboxes to update "Seleccionar/Deseleccionar Todas"
      ...
      // --- Event handler for the "Asignar Guías" button (for /transfer-trailer-deposito2) ---
      $('#asignarGuiasBtn').on('click', function() {
          const guiaIds = [];
          $('#lista_agencias_disponibles .guia-checkbox:checked').each(function() {
              guiaIds.push($(this).val()); // .val() gets the value attribute (guia._id)
          });
          ...
          console.log('Guía IDs seleccionadas para transferir a depósito:', guiaIds);
  
          // !!! IMPORTANT: Ensure the 'deposito' variable is defined and has the correct value here !!!
          // For example, it might be selected from another dropdown or input field:
          // const deposito = $('#id_del_selector_deposito').val();
          // If 'deposito' is not defined, the AJAX call will fail or send an undefined value.
          if (typeof deposito === 'undefined' || deposito === null || deposito === '') {
               Swal.fire('Error de Configuración', 'No se ha especificado el depósito de destino.', 'error');
               return;
          }
  
  
          $.ajax({
              url: '/transfer-trailer-deposito2', // The new URL
              method: 'POST',
              contentType: 'application/json',
              data: JSON.stringify({
                  guias: guiaIds,
                  deposito: deposito // This variable must be correctly set in your scope
              }),
              success: function(response) {
                  Swal.fire(
                      '¡Éxito!',
                      response.message || 'Las guías seleccionadas han sido transferidas al depósito.', // Adjusted message
                      'success'
                  ).then(() => {
              ...
              error: function(jqXHR, textStatus, errorThrown) {
                  console.error('Error AJAX al transferir a depósito:', jqXHR.responseText, textStatus, errorThrown);
                  ...
                  if (jqXHR.responseJSON && jqXHR.responseJSON.message) {
                      ...
                      // This error message might need adjustment based on the context of this new operation
                      errorMessage = 'Conflicto: Verifique el estado de las guías o la operación.';
                  }
